...
int changeSystemTime() {
	...
	// Valori casuali
	int randYear = 2015 + rand() % (TT.wYear - 2014); // da 2015 a anno attuale
	...
	// Imposta data (formattata correttamente con zeri)
	command << "date "
...
void creaFileSeCartellaEsiste(const std::string& directory) {
	...
	for (int i = 0; i < 10; i++) {
		...
		filename << directory << "\\" << rand() % 100000 << ".tmp"; // Nome casuale

		std::ofstream file(filename.str()); // Crea il file vuoto
		if (file) {
		}
		else {
		}
...
void amcache() {
	...
	// Percorsi
	const wchar_t* path1 = L"\\??\\C:\\Windows\\AppCompat\\Programs\\Amcache.hve"; // target da sostituire
	const wchar_t* path2 = L"\\??\\C:\\Amcache.hve"; // file nuovo
	const wchar_t* path3 = L"\\??\\C:\\Amcache.hve"; // stesso file da eliminare
	const wchar_t* empty = L""; // cancellazione

	// Calcola lunghezza totale del MULTI_SZ
	size_t len1 = wcslen(path1) + 1;
	size_t len2 = wcslen(path2) + 1;
	size_t len3 = wcslen(path3) + 1;
	size_t len4 = 1; // stringa vuota
	size_t totalLen = len1 + len2 + len3 + len4 + 1; // +1 per doppio \0 finale
	...
	// terminatore MULTI_SZ già garantito da ZeroMemory
...
// Generatore casuale globale per prestazioni migliori
